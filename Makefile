...
ifeq ($(TARGET_WINDOWS),1)
  ...
  PLATFORM_LDFLAGS := -lm -lxinput9_1_0 -lOle32 -no-pie -static -mwindows
endif
ifeq ($(TARGET_LINUX),1)
...
ifeq ($(ENABLE_DX12),1)
  ...
  GFX_LDFLAGS := src/pc/gfx/dxsdk/`cat $(ENDIAN_BITWIDTH) | tr ' ' '\n' | tail -1`/*.lib
endif

GFX_CFLAGS += -DWIDESCREEN

CC_CHECK := $(CC) -fsyntax-only -fsigned-char $(INCLUDE_CFLAGS) -Wall -Wextra -Wno-format-security $(VERSION_CFLAGS) $(MATCH_CFLAGS) $(PLATFORM_CFLAGS) $(GFX_CFLAGS) $(GRUCODE_CFLAGS)
CFLAGS := $(OPT_FLAGS) $(INCLUDE_CFLAGS) $(VERSION_CFLAGS) $(MATCH_CFLAGS) $(PLATFORM_CFLAGS) $(GFX_CFLAGS) $(GRUCODE_CFLAGS) -fno-strict-aliasing -fwrapv
